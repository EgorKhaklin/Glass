...
The field and hash are now real, but two **structural soundness gaps** remain — this is why
it is **research-grade, not production**: (i) the prover's inline Fiat-Shamir transcript
binds the FRI commitments (layer roots) but **not yet the statement** (circuit digest +
claimed result + public inputs); (ii) there is **no separate `verify(proof, public)`** —
the prover self-checks over the witness, which does not yet bound a *malicious* prover. It
proves the idea end to end on the production field + hash; it is **not** a tool for
protecting secrets in production.
...
3. **Fiat-Shamir rigor** — domain separation exists (`frost_goldilocks_fiat.glass`), but the
   prover's inline transcript must **bind the statement** (circuit digest + claimed result +
   public inputs, not just the FRI roots), plus a formal transcript-separation / soundness
   argument. *Still to do.*
