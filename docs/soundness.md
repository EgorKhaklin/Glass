...
|---|---|---|
| **Base field** | Baby Bear (2³¹−2²⁷+1) is a genuine NTT-friendly prime; a Glass circuit is now also proven **succinctly + ZK over Goldilocks (2⁶⁴)** — the gate quotient `Q=G/Z_H` FRI-tested + blinded (`prove_circuit_goldilocks_zk.glass`) | The **source bridge** (`prove_source_*`) still computes over Baby Bear, so its values are capped near 2³¹. The Goldilocks circuit proof (R1b) removes that cap for a hand-built circuit; wiring it into the full source bridge — and adding the F_{p²} fold challenge + Merkle queries to it — is the remaining work. |
