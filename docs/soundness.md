...
| **Hash** | **Poseidon over Goldilocks, byte-identical to Plonky2** — the de-facto standard Goldilocks ZK hash — verified against **Plonky2's own published test vectors** (all four pass): t=12, R_F=8, R_P=22, S-box x⁷, Plonky2's exact MDS and all 360 round constants (the Poseidon reference's "hadeshash" Grain-LFSR constants), reproduced in `frost_goldilocks_poseidon.glass` and dogfooded byte-identical. It is also **load-bearing**: `frost_goldilocks_merkle.glass` builds a real Merkle commitment on it (Plonky2's exact `two_to_one` + `hash_no_pad`, inclusion proofs, tamper-rejection). (Also `frost_grain.glass`: a from-scratch Grain-LFSR generator + domain-separated transcript over Baby Bear.) | This settles "is it the standard hash" for the **primitive**: it matches a production reference exactly, and now drives a commitment. **But it is not yet integrated into the STARKs** — their Merkle + Fiat-Shamir still use the educational MiMC. **Wiring it into the prover was attempted, and is blocked by the *runtime*, not the structure:** the 2-to-1 Poseidon `hashg` (`perm([a,b,0…])[0]`) works and dogfoods byte-identical at small scale, but the full FRI prover **OOMs** — even *one* minimal proof (64-pt coset, ~190 permutations) exceeds 16 GB, because the native Glass runtime is **no-free** (it never calls `free()`/GCs) and each Poseidon permutation allocates ~300× a MiMC round, so the allocations accumulate past memory. The *same* prover with MiMC completes (it ran 2h07m). So Poseidon-in-the-prover needs a **freeing/GC runtime** (Python GCs, so it could run interpreted alone — but the dogfood needs the native binary too) or the future int64-erased backend; it's a runtime limit, not a soundness one. And matching a reference is **not an audit**: the hash itself is unaudited here. |
