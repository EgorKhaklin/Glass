...
|---|---|---|
| **Base field** | Baby Bear (2³¹−2²⁷+1) is a genuine NTT-friendly prime; a Glass circuit is now also proven over **Goldilocks (2⁶⁴)** by a full cryptographic STARK — gate quotient `Q=G/Z_H`, embedded in F_{p²}, blinded, Merkle-committed, F_{p²}-challenged (Fiat-Shamir), and query-verified (`prove_circuit_goldilocks_zk.glass`) | The **source bridge** (`prove_source_*`) still computes over Baby Bear, so its values are capped near 2³¹. The Goldilocks circuit proof (R1b) removes that cap for a hand-built circuit with a real STARK; wiring it into the full source bridge is the remaining step. |
