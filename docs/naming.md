# Naming — the plain surface and the branded names

Several Glass features have two names. The **plain name** is the public, professional
surface — what you type, what `glass help` lists, what a newcomer reads. The **branded
name** is a memorable label for the same feature. Both work on the CLI; neither is forced
on the other.

If you want Glass to read as a plain verifiable-computing toolkit, use the left column
and ignore the rest.

| Plain name (the surface) | Branded name | What it actually is |
|---|---|---|
| `glass verify` · the independent verifier (`pentecost/`) | Pentecost · "let the verifier be two" | A second, from-scratch STARK verifier (plain `int mod p`, no Glass code) that re-checks a proof — catches an implementation bug in the primary verifier that self-consistency cannot. |
...
Renaming the directories would break the Name's path-bindings and the test suite, so the
