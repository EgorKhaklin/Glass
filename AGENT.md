...
Glass is a pure functional programming language. The implementation is a tree-walking interpreter in Python (~2900 lines, single file: `glass.py`) plus a Glass-in-Glass self-host (~4200 lines: `examples/selfhost/prism.glass`). The language is at v2.x — "matured" — and the destination is v3.0 ("Quartz", native compilation, glass.py becomes optional).
...
- **The language describes itself.** prism.glass is a Glass interpreter written in Glass, run by glass.py today, compiled natively in v3.0.
...
prism.glass is the host being rewritten in Glass. Every release should either *advance* this (close a feature gap, add a builtin, fix a parser quirk) or *avoid breaking it* (don't add host features that prism can't easily mirror). The destination is v3.0 = Quartz = glass.py becomes optional.
...
### Top-level `let` followed by parenthesized expression parses as application

```glass
let v = f(x)
(g(y))                # this is f(x)(g(y)), NOT a separate top-level expression
```

Workaround: use `let v = ... in (...)` to bound the let's scope, or insert a top-level expression that's not parenthesized.
...
### Sequential top-level lets + generic fns

A known host limitation (pre-existing, surfaced during v2.9 generic-fn work): sequential top-level `let` bindings that each instantiate the same generic fn at a different type fail to type-check. Specifically:

```glass
fn id<A>(x: A) : A = x

let n = id(42)
let s = id("hello")   # error: not a function: String
(n, s)
```

The first call binds `id` to `Int -> Int` and the scheme doesn't refresh for the second binding. The workaround is to call each instantiation inside a single expression:

```glass
let result = (id(42), id("hello"))   # works fine
```

This is a host bug, not a Glass language issue — generic fns are polymorphic when called within a single expression, and prism handles both shapes correctly. Fix candidate for a future host patch: re-instantiate Schemes on each name lookup at top level.
...
| v3.x  | **Quartz.** Native compilation. glass.py becomes optional. |
| v4.x  | **Pane.** Query layer (graph queries over typed data). |
| v5.x  | **Frost.** ZK extension of Pane. |
...
When in doubt about whether something is a 2.x or 3.x: if it's the *destination* (native compile, query layer, ZK), it's a major bump. If it's a *step toward* the destination, it's a 2.x minor. If it's a *fix* of an earlier 2.x release, it's a 2.x.y patch.
