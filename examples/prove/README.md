...
- [`prove_pane.glass`](prove_pane.glass) — **Frost as a second backend over the real Pane query algebra.** Take an actual Pane `Query` value (`SumQ`, `CountQ`, `Where(…)`) and lower it gadget-for-gadget into a circuit — equality, booleans (`And`/`Or`/`Not`), arithmetic, and **order comparisons** (`<`/`>`, via a 17-bit range gadget). One AST, two evaluators: `run_query` *interprets* it, `prove_pane` *proves* it — and the discipline is that they agree, the circuit ACCEPTing exactly when its answer equals `run_query`'s.
